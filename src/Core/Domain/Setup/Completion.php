 ...
 * wizard is still open is read off the accounts (decision O3 in
 * .ai/plans/23-instalace-na-zelene-louce.md: open until somebody administers
 * the installation), and that reading is asked before anybody writes - which
 * is exactly why it cannot be what decides between two visitors finishing at
 * the same moment: both would ask, both would be told "nobody yet", and both
 * would become the installation's administrator. What decides is this row.
 * Trilobit\Core\Setup\Installer writes it first, inside the transaction that
 * makes the account and the business, and its key is the primary key: the
 * second visitor's insert waits on the first one's row and is refused when
 * that commits, and the second visitor writes nothing at all.
 *
 ...
 * one: the wizard is closed by the account those commands made, not by this.
 *
